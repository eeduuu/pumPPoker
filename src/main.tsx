...
function App() {
 ...
 const enterTable = () => {
  ...
  try { const hand = createInitialHand(c); setTableHand(hand); setTableSession(value=>value+1); setTableError(''); }
  catch { opening.current = false; setTableError('No se pudo crear el reparto seguro. Vuelve a intentarlo.'); }
 };
 const restartTable=()=>{
  try{opening.current=true;setTableHand(createInitialHand(c));setTableSession(value=>value+1);setTableError('');}
 ...
 if (tableHand) return <Table key={tableSession} config={c} hand={tableHand} onRestart={restartTable} onLeave={() => { opening.current = false; setTableHand(null); setStep(0); }}/ >;
 return <main className="shell"><header><a className="brand" href="#" onClick={e => e.preventDefault()}><BrandLogo/></a><span className="free"><i/> SIN DINERO REAL</span></header>
 <section className="wizard" aria-label="Crear partida"><div className="eyebrow">TEXAS HOLD’EM <span>•</span> TÚ CONTRA LOS BOTS</div><nav aria-label="Progreso"><ol>{['Partida', 'Jugadores', 'Ritmo', 'Resumen'].map((name, i) => <li key={name} className={i === step ? 'active' : i < step ? 'done' : ''} aria-current={i === step ? 'step' : undefined}><span>{i < step ? '✓' : i + 1}</span><small>{name}</small></li>)}</ol></nav>
 ...
 {step === 2 && <><div className="fields">{numeric('small', 'Ciega pequeña', 1, Math.min(c.chips, c.big - 1))}{numeric('big', 'Ciega grande', c.small + 1, c.chips)}</div>{c.mode === 'normal' && <label className="check"><input type="checkbox" checked={c.growing} onChange={e => set('growing', e.target.checked)}/> Aumentar ciegas por niveles</label>}{(c.mode === 'tournament' || c.growing) && <div className="fields">{numeric('minutes', 'Minutos por nivel', 1, 120)}{c.mode === 'tournament' && <label className="check"><input type="checkbox" checked={c.breaks} onChange={e => set('breaks', e.target.checked)}/> Descansos</label>}</div>}{c.mode === 'tournament' && c.breaks && <div className="fields">{numeric('every', 'Descanso cada … niveles', 1, 50)}{numeric('rest', 'Minutos de descanso', 1, 60)}</div>}<div className="note">{c.mode === 'normal' && !c.growing ? 'Las ciegas se mantienen durante toda la partida.' : 'Las ciegas se duplican por nivel, entre manos. El reloj se pausa al ocultar la pestaña.'}</div></>}
 {step === 3 && <><dl><div><dt>Modalidad</dt><dd>{c.mode === 'normal' ? 'Partida normal' : 'Torneo'}</dd></div><div><dt>Jugadores</dt><dd>Tú + {c.bots} {c.bots === 1 ? 'bot' : 'bots'}</dd></div><div><dt>Fichas iniciales</dt><dd>{c.chips.toLocaleString('es-ES')} por jugador</dd></div><div><dt>Ciegas</dt><dd>{c.small} / {c.big} · {c.mode === 'normal' && !c.growing ? 'Fijas' : `${c.minutes} min/nivel`}</dd></div><div><dt>Ritmo</dt><dd>Rápido · acciones visibles paso a paso</dd></div>{c.mode === 'tournament' && <div><dt>Descansos</dt><dd>{c.breaks ? `${c.rest} min cada ${c.every} niveles` : 'Desactivados'}</dd></div>}</dl><p className="note">Al crear la mesa se reparten tus cartas automáticamente. Ritmo rápido, sin saltarse el reparto ni los turnos.</p></>}
 </div>{tableError && <p className="note" role="alert">{tableError}</p>}<footer><button className="back" type="button" disabled={step === 0} onClick={() => move(step - 1)}>← Atrás</button><button className="next" type="submit">{step === 3 ? 'Crear mesa' : 'Siguiente'} <span>→</span></button></footer></form></section><div className="bottom">HECHO PARA JUGAR, NO PARA APOSTAR.<span>CONFIGURA TU PARTIDA</span></div></main>;
}
