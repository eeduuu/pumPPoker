...
export function Table(props:TableProps){
 ...
 const [eliminationActive,setEliminationActive]=useState(false);
 ...
 const {elapsed,remaining,reset}=useLevelClock(duration,enabled,stopped||!!intermission||eliminationActive);
 ...
 return <HandTable key={number} {...props} breakLabel={breakLabel} config={{...props.config,small:level.small,big:level.big}} levelLabel={levelLabel} onTerminal={setStopped} onEliminationActive={setEliminationActive} profiles={profiles} hand={hand} number={number} onAdvance={advance} advanceError={error}/>;
}
function HandTable({ config, hand, playerName, onLeave,onRestart,number,onAdvance,advanceError,profiles,levelLabel,onTerminal,onEliminationActive,breakLabel }: TableProps&{number:number;onAdvance:(stacks:number[])=>void;advanceError:string;profiles:BotProfile[];levelLabel:string;onTerminal:(value:boolean)=>void;onEliminationActive:(value:boolean)=>void;breakLabel:string}) {
 ...
 useEffect(()=>{
  ...
  useEffect(()=>{onEliminationActive(eliminating);return()=>onEliminationActive(false);},[eliminating,onEliminationActive]);
 ...
 useEffect(()=>{
  ...
  const resume=()=>{
   ...
   if(!celebrationAnnounced.current){celebrationAnnounced.current=true;feedback('win',terminalWinner===0);}
   setCelebrating(true);
 ...
 },[config.mode,terminalWinner,eliminating,feedback]);
 ...
 useEffect(()=>{
  if(canAct&&!wasHumanTurn.current)feedback('turn',true);
 ...
 const dockStatus=outcome.terminal?'':spectating?(ended?endNotice:'Modo espectador · El torneo continúa.'):ended?endNotice:frame.done&&betting.status!=='playing'?(betting.status==='uncontested'?'Siguiente mano en unos segundos…':community.street===3?'Repartiendo el bote…':'Preparando '+['flop','turn','river'][community.street]+'…'):'';
  ...
  <section className={"table-stage seat-count-"+config.bots+(config.bots>=5?" dense-table":"")+(config.bots>=7?" crowded-table":"")} aria-label="Mesa de póker"><div className="felt" aria-hidden="true"/>
   {layouts[config.bots].map((position, i) => {
    const [x,y]=compareHands?revealLayouts[config.bots][i]:position;
    const isTop=position[1]===9;
    ...
    return <article className={"bot-seat"+(active===seat?" receiving":"")+(payout>0?" winner-seat":"")+(eliminated?" eliminated":"")+(compareHands&&reveal?" reveal-showing":"")+(compareHands&&reveal&&isTop?" reveal-top":"")+(compareHands&&reveal&&isTop&&(config.bots===3||config.bots===7)?" reveal-top-three":"")} data-hand-status={seatStatus??undefined} data-seat={seat} key={i} style={{ left: `${x}%`, top: `${y}%` }} aria-label={`Bot ${seat}, ${stack(seat)}${eliminated?', eliminado del torneo':seatStatus==='active'?', activo en la mano':seatStatus==='folded'?', retirado de esta mano':''}${allIn?', all-in':''}${payout>0?', cobra '+bb(payout):''}`}>
