# ♠ pumꟼPoker

[▶ **Jugar ahora**](https://eeduuu.github.io/pumPPoker/)

**Juegos gratuitos, rápidos y sin dinero real.**

Un proyecto personal creado por mí desde cero para disfrutar de partidas sencillas desde cualquier dispositivo. La colección seguirá creciendo con nuevos juegos y mejoras.

No necesitas registrarte ni descargar nada para jugar desde el navegador.

## Tener la app en el móvil

- **Android:** abre la web en Chrome, pulsa el menú y elige **Añadir a pantalla de inicio** o **Instalar aplicación**.
- **iPhone:** abre la web en Safari, pulsa **Compartir** y elige **Añadir a pantalla de inicio**.

Después podrás abrirla desde su icono como cualquier otra app.

---

Hecho con dedicación para jugar, aprender y seguir mejorando.
