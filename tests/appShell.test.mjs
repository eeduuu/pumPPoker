...
test('Las eliminaciones se presentan de una en una durante cinco segundos y pausan la mesa', async () => {
  ...
  assert.match(animation, /ELIMINATION_MS = 5000/);
  assert.match(animation, /feedback\('ignite'\)/);
  assert.match(animation, /feedback\('blast', true\)/);
